...
export default new class ServerManager {
    ...
    async ultraSpecialAndIncrediblyStupidHandlerForQuilt(serverVersion: string, installerVersion: string, loaderVersion: string, path: string) {
        ...
        //
        // UPDATE: I NOW HAVE EVEN MORE BEEF WITH QUILT
        // WHY. IN THE EVERLIVING *F*CK* DO WE NEED A 𝗟𝗔𝗨𝗡𝗖𝗛𝗘𝗥 FOR A SERVER SOFTWARE??
        // QUILT IS THE ONLY SOFTWARE THAT DOES THIS, EVERY SINGLE OTHER ONE WORKS PERFECTLY.
        // I HAVE TO MAKE CUSTOM BULLSH*T JUST FOR QUILT. F*CK QUILT. BEST PART - IT ONLY BREAKS
        // WHEN YOU HAVE MODS (AND DOESN'T EVEN TELL YOU A DESCRIPTIVE MESSAGE, IT'S JUST SOME DATAPACK BULLSH*T).
        ...
        return new Promise((res, rej) => {
            ...
            if (ivm !== null && ivm[0] == installerVersion && lvm !== null && lvm[0] == loaderVersion) {
                ...
                stupid.on('close', async (code) => {
                    ...
                    if (await exists(`${path}/quilt-server.jar`)) await fs.rm(`${path}/quilt-server.jar`);
                    await fs.rename(`${path}/server.jar`, `${path}/quilt-server.jar`);
                    await fs.rename(`${path}/quilt-server-launch.jar`, `${path}/server.jar`);
                    await fs.writeFile(`${path}/quilt-server-launcher.properties`, `# Auto-generated by BurgerPanel\n#${new Date().toUTCString().split(" ").slice(0, 5).join(" ")}\nserverJar=quilt-server.jar\n`);
                    if (code != 0 && code !== null) {
                        res(false);
                    } else {
                        res(true);
