<script setup lang="ts">
    ...
    async function createArchive(path: string, folderName: string) {
        let rnd_arr = new Uint32Array(1);
        let rnd = crypto.getRandomValues(rnd_arr);
        let time = new Date();
        let time_str = `${time.getFullYear()}-${time.getMonth().toString().padStart(2, "0")}-${time.getDay().toString().padStart(2, "0")}`;
        let rnd_str = "";
        rnd_arr.forEach(item => {
            rnd_str += item.toString(16);
        });
        let name = `${folderName}_${time_str}_${rnd_str}.tar.gz`;
        if((await requestModal({
            confirmButtonType: "CONFIRM",
            title: "Are you sure you want to archive this?",
            description: `This will create an archive with the name \`${name}\` of this folder.\nThis is useful if you want to download folders without downloading every file inside it.`
        })).type == "YES") {
            let resp = await sendRequest("serverFiles", {
                path,
                id: props.server,
            ...
            if(resp.type != "archiveSuccess") return event.emit("createNotification", `Couldn't create archive ${name}, something went wrong.`);
            event.emit("createNotification", `Archive created as ${name}`);
            await getFiles();
        }
    }
    async function extractArchive(path: string, fileName: string) {
        if((await requestModal({
            confirmButtonType: "CONFIRM",
            title: "Are you sure you want to extract this?",
            description: `This will extract the archive \`${fileName}\`.\nNOTE: This will extract the CONTENTS of the archive into the current folder (if you archived a folder, the contents of the folder got archived, not the folder itself). If you don't want that, move it into a separate folder first.`
        })).type == "YES") {
            event.emit("createNotification", `Extracting archive...`);
            let resp = await sendRequest("serverFiles", {
                path,
                id: props.server,
                file_name: fileName,
            ...
            if(resp.type != "extractSuccess") return event.emit("createNotification", `Couldn't extract archive ${fileName}, something went wrong.`);
            event.emit("createNotification", `Archive ${fileName} extracted`);
            await getFiles();
        }
    }
        ...
        <Dropdown :create-on-cursor="true" ref="dropdown">
            <div id="dropdown-inner">
                ...
                <button v-if="hasServerPermission(user.user, server, 'serverfiles.delete')/* && !dropdownFile.folder*/" @click="async () => {
                    dropdown.hide();
                    if(await confirmModal(`Delete ${dropdownFile.name}?`, `Are you sure you want to delete ${dropdownFile.name}? This can't be undone.`, true, true, true)) {
                        await sendRequest('serverFiles', {
                            id: props.server,
                            action: 'delete',
                ...
                <button v-if="hasServerPermission(user.user, server, 'serverfiles.archive') && dropdownFile.folder" @click="createArchive(path.toString(), dropdownFile.name)">Create Archive</button>
                <button v-if="hasServerPermission(user.user, server, 'serverfiles.extract') && !dropdownFile.folder && dropdownFile.name.endsWith('.tar.gz')" @click="extractArchive(path.toString(), dropdownFile.name)">Extract</button>
