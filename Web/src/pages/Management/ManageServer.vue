    ...
    <div v-if="server" class="console">
        <div class="console-header">
            ...
            <div class="console-buttons">
                <div class="button"><button style="margin-left:40px" @click="startServer()" :disabled="!user.hasServerPermission(server, 'start') || isRunning" id="startbtn">Start</button></div>
                <div class="button"><button @click="stopServer()" :disabled="!user.hasServerPermission(server, 'stop') || !isRunning || thisServerStatus == 'stopping'">Stop</button></div>
                <div class="button"><button @click="killServer()" :disabled="!user.hasServerPermission(server, 'kill') || !isRunning">Kill</button></div>
                <div class="button"><RouterLink :to="{name: 'editServer', params: {server: server._id}}"><button>Edit</button></RouterLink></div>
            </div>
